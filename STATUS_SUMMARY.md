...
- fixture 기반 시나리오 확장은 baseline 변경 전 별도 설계가 필요하며, 현재 Queue payload validator와 live-read checks는 standalone manual tooling boundary까지 정리됐고 submit normalization / Notion mapping은 submit fixture validator design까지 완료됐다.
...
## 지금 바로 수정해도 안전한 항목
- submit fixture validator 구현 승인 여부를 별도 docs-only decision으로 검토
