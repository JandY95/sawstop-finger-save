...
- fixture 기반 시나리오 확장은 baseline 변경 전 별도 설계가 필요하며, 현재 Queue payload fixture JSON 리뷰가 다음 경계다.
...
## 지금 바로 수정해도 안전한 항목
- Queue payload fixture JSON 리뷰 결과를 바탕으로 runner 연결 여부 판단
- runner/compare, `parity-baseline.json`, `scenario-index.yaml` 변경은 별도 승인 전까지 보류
